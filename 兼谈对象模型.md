...
public:
    // 构造函数
    Fraction(int num, int den=1)  // Fraction(int num, int den=1)
...
让2，3，4这三种情况下的隐式转换规则和原因：

1. 第一种情况 `double d = 4 + f` 能够工作是因为：
- 这里只有一个可能的转换路径：通过 `operator double()` 将 Fraction 转换为 double
- C++ 知道要进行内置类型 double 的加法运算，所以会尝试将操作数转换为 double
- 找到了 `operator double()` 转换函数，可以将 Fraction 转为 double
- 转换路径唯一且明确

2. 第二种情况 `Fraction d2 = f + 4` 能够工作是因为：
- 这里只有一个可能的转换路径：通过构造函数将 4 转换为 Fraction
- 编译器发现有 `operator+(const Fraction&)`，需要两个 Fraction 类型的操作数
- 找到了可以将 int 转换为 Fraction 的构造函数
- 转换路径唯一且明确

3. 第三种情况不能工作，会报错"ambiguous"(模糊)是因为：
- 存在两条可能的转换路径：
  1. 将 4 转换为 Fraction (通过构造函数)，然后用 `operator+`
  2. 将 f 转换为 double (通过 operator double())，然后和 4 进行 double 加法
- 编译器无法决定使用哪条转换路径，因为两条路径都是合法的
- 这种情况下需要显式指定转换方式，比如：
  ```cpp
  Fraction d2 = f + Fraction(4);  // 明确使用构造函数转换
  // 或
  double d2 = double(f) + 4;      // 明确使用 operator double() 转换
  ```

总结：C++ 允许隐式转换，但要求转换路径必须是唯一且明确的。当存在多条可能的转换路径时，就会产生歧义，需要程序员显式指定使用哪种转换。
...
在 C++ 中，`operator*` 和 `operator->` 都是运算符重载的一部分。它们的重载允许你自定义如何通过对象使用指针解引用（`*`）和成员访问（`->`）的操作。

### 1. `operator*` 的重载

`operator*` 通常用于重载解引用运算符，这使得你能够像指针一样使用对象（即解引用对象）。这通常用于类对象模仿指针行为的场景。
...
private:
    int value;
public:
    MyPointer(int val) : value(val) {}

    // 重载 operator*，允许通过 MyPointer 对象解引用获取值
    int operator*() {
        return value;
    }
};
...
在这个例子中，`*obj` 被重载为返回 `value` 的值，而不是指针的解引用。

### 2. `operator->` 的重载

`operator->` 用于重载箭头运算符，它允许你通过对象访问类成员。通常它用于智能指针类中，使得它们可以像原始指针一样使用 `->` 访问成员。
...
int main() {
    MyPointer obj(20);
    cout << *obj.operator->() << endl;  // 输出: 20
    return 0;
}
```

在这个例子中，`operator->` 被重载为返回 `value` 的地址，从而允许通过箭头操作符访问该成员。

## 8.迭代器

C++中的**迭代器（Iterator）**是一种用于访问容器（如数组、链表、向量等）中元素的对象。它充当了容器与算法之间的桥梁，使得对容器内的元素进行访问变得更加灵活和通用。迭代器的作用类似于指针，它可以在容器内“遍历”元素，从而使得在不同类型的容器上应用相同的算法成为可能。

### 1. **迭代器的作用**

- 迭代器提供了统一的接口来访问容器中的元素。无论容器的内部实现是什么，迭代器都能提供一致的方式来遍历容器。
- 通过迭代器，可以避免直接使用容器特有的下标或指针，从而提高代码的可移植性和灵活性。
- 迭代器与容器的解耦使得容器和算法之间的配合变得更加通用和独立。

### 2. **常见的迭代器类型**

C++标准库定义了几种不同类型的迭代器，它们根据功能的不同，具有不同的权限和能力：

- **输入迭代器（Input Iterator）**：只能读取元素（单向）。
- **输出迭代器（Output Iterator）**：只能写入元素（单向）。
- **前向迭代器（Forward Iterator）**：可以读取和写入元素，且可以多次遍历。
- **双向迭代器（Bidirectional Iterator）**：支持在容器中前后移动。
- **随机访问迭代器（Random Access Iterator）**：支持像数组一样的任意位置访问，可以进行加减操作。

### 3. **常见的迭代器操作**

迭代器支持一些常用的操作，类似于指针操作：

- **解引用（`\*`）**：获取当前迭代器指向的元素。
- **移动迭代器（`++` 或 `--`）**：使迭代器向前或向后移动。
- **比较迭代器（`==`, `!=`）**：判断两个迭代器是否相等。
- **访问元素的成员**：对于某些容器（如 `map`、`set`），可以使用 `->` 来访问元素的成员。
