...
在 C++ 中，`private`、`public` 和 `protected` 是**访问控制修饰符**，用于控制类成员（包括属性和方法）的访问权限。这些修饰符用于控制类内部的成员变量或成员函数对外部代码的可见性和可访问性。访问控制是面向对象编程中的一个重要概念，用来实现封装（Encapsulation）这一特性。

### 1. `public`（公共访问权限）

- **访问权限**：类的所有成员（数据成员和成员函数）如果被声明为 `public`，则可以在类外部、类的任何对象以及子类中直接访问。
- **典型使用场景**：通常用于类的接口部分，也就是希望提供给外界使用的方法和属性。
...
int main() {
    MyClass obj;
    obj.publicVar = 20;  // 可以在类外部访问并修改 public 变量
    obj.publicMethod();  // 可以在类外部调用 public 方法
}
```

### 2. `private`（私有访问权限）

- **访问权限**：类的所有成员如果被声明为 `private`，则只能在类的内部（同一个类的成员函数中）访问。即使是类的派生类（子类）或类的外部代码也无法访问这些私有成员。
- **典型使用场景**：用于隐藏类的实现细节，保护类的内部状态，不让外界直接修改，避免类外部的代码对类的内部实现产生不良影响。
...
public:
    void setPrivateVar(int value) {  // 通过 public 函数修改 private 变量
        privateVar = value;
    }

    int getPrivateVar() {  // 通过 public 函数访问 private 变量
        return privateVar;
    }
};
...
int main() {
    MyClass obj;
    // obj.privateVar = 20;  // 错误：无法在类外部访问 private 变量
    obj.setPrivateVar(20);  // 正确：通过 public 方法访问 private 变量
    int val = obj.getPrivateVar();  // 正确：通过 public 方法获取 private 变量值
}
```

### 3. `protected`（受保护的访问权限）

- **访问权限**：`protected` 成员的访问权限介于 `private` 和 `public` 之间。`protected` 成员可以被该类自身及其派生类（子类）访问，但在类的外部仍然无法直接访问。
- **典型使用场景**：通常用于继承体系中，允许子类继承并访问父类的成员，而不允许类外部的代码访问这些成员。
...
public:
    void setProtectedVar(int value) {  // 公共方法修改 protected 变量
        protectedVar = value;
    }
};
...
public:
    void accessProtected() {
        protectedVar = 20;  // 子类可以访问基类的 protected 成员
    }
};
...
int main() {
    ...
    // baseObj.protectedVar = 30;  // 错误：不能在类外部访问 protected 成员
    derivedObj.accessProtected();  // 正确：子类方法可以访问基类的 protected 成员
}
```

### 4. `private`、`public` 和 `protected` 在继承中的差异

当一个类继承另一个类时，基类的访问控制修饰符会影响派生类对基类成员的访问权限。基类成员的访问权限在派生类中可能发生变化，取决于继承方式（`public`、`protected` 或 `private` 继承）：
