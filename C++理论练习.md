...
在C++的一个类中，如果构造函数**没有参数**，或者构造函数的**所有参数都有默认值**，就可以称其为缺省构造函数。一个类中，**只能有一个缺省构造函数**。

关于内联函数：所有成员函数默认为内联函数。

构造函数不返回任何值，这与“返回viod” 不是一个意思
...
1. **名称与类名相同**。
2. **没有返回值**，也不能声明为 `void`。
3. **自动调用**：创建对象时由编译器自动调用。
4. 默认构造函数：
   - 无参数构造函数。
   - 当用户未定义任何构造函数时，编译器会自动生成默认构造函数。
5. 支持重载：
   - 可以定义多个构造函数，参数列表必须不同。
6. 可以是内联函数：
   - 简单的构造函数可以声明为内联。
7. 编译器会默认生成：
   - 如果没有定义任何构造函数，编译器会提供隐式的默认构造函数和拷贝构造函数。
...
1. 如果类定义了带参构造函数，但未定义无参构造函数，则编译器不会生成默认构造函数。
2. 拷贝构造函数用于以下场景：
   - 对象通过值传递时。
   - 对象作为函数返回值时。
   - 显式创建一个对象副本时。
3. 委托构造函数通过调用其他构造函数来简化初始化逻辑。
...
2. **拷贝构造函数与深拷贝**：
   ...
   - 默认拷贝构造函数只执行浅拷贝。
   - 深拷贝需要手动实现，避免共享指针、动态内存等问题。

3. **委托构造函数**：

   - 一个构造函数调用另一个构造函数实现初始化：
...
1. 构造函数不能有返回值，也不能指定为 `void`。
2. 如果类定义了带参构造函数而没有无参构造函数，创建无参对象时会报错。
3. 如果类有动态分配的资源（如指针），需要手动实现拷贝构造函数和析构函数，否则可能引发悬挂指针问题。
...
1. **没有返回值**：构造函数不返回任何值。
2. **支持重载**：通过不同的参数列表实现不同的初始化逻辑。
3. **初始化列表**：用于效率更高地初始化成员变量。
4. **编译器生成默认构造函数**：仅当用户未定义任何构造函数时。
5. **深拷贝与浅拷贝**：默认构造函数进行浅拷贝，需手动实现深拷贝以管理动态资源。
...
解答：
 ...
 B 选项描述不正确，指针变量可以通过求地址运算符（ &) 来获得地址值，可以通过指针变量获得地址值，还可以通过标准函数获得地址值； C 选项中， p=NULL ；和 p=0 ；或 p='\0' ；等价； D 选项中，语句 p=NULL; 执行后，指针 p 并不是指向地址为 0 的存储单元，而是具有一个确定的值 -" 空 " 。因此 A 选项正确。

### 知识点总结

**未初始化的指针**

未初始化的指针在声明后，其值是随机的（通常是内存中的垃圾值），它们可能指向任何未知的内存地址。未初始化的指针的行为是未定义的，一旦尝试解引用（比如 `*p1`），程序可能会崩溃或产生不可预测的行为。
...
- 已被释放的内存地址。
- 非法的内存区域（比如某些受保护的系统内存区域）。
- 任意地址，而没有经过合理的初始化。

未初始化的指针和野指针的关系可以总结如下：

- 未初始化的指针**有可能**是野指针，因为它没有明确指向有效的内存地址。
- 一旦指针被明确初始化（即指向有效的内存），就不再是野指针。

**指针的定义**：

- 指针变量可以用 `*` 定义，并需明确类型（如 `int*`, `char*`）。
- 指针变量初始值为随机值，需初始化（如指向有效地址或 `NULL`）。

**指针赋值**：

- 可以通过 `&` 运算符、动态内存分配、或直接赋值地址。
- 空指针（`NULL` 或 `0`）表示指针未指向任何对象。
...
在C++中，定义函数时可以给形参指定一个默认的值，这样调用函数时如果没有给这个形参赋值（没有对应的实参），那么就使用这个默认的值。也就是说，调用函数时可以省略有默认值的参数。如果用户指定了参数的值，那么就使用用户指定的值，否则使用参数的默认值。默认参数可以有多个。
...
- **默认参数的规则**：
  ...
  1. 函数的参数可以有默认值，提供默认值可以让函数调用时省略某些参数。

  2. 一旦某个参数被设定了默认值，其后的所有参数必须设定默认值

     ，以保证调用时参数的数量和对应关系是明确的。例如：

     ```cpp
     void example(int a, int b = 10, int c = 20); // 合法
     void example(int a = 5, int b, int c = 20); // 不合法，b 没有默认值
     ```
...
- **默认值的指定位置**：
  ...
  - 默认值通常只在
    ...
    ```cpp
    void func(int x, int y = 10); // 声明中给 y 默认值 10
...
- `this` 是用于引用当前对象的关键字。
- 可以在**实例方法**中使用 `this` 来引用当前对象的属性或调用实例方法。
- 在**静态方法**中没有 `this`，因为静态方法不属于特定的对象，而是属于整个类。

**2. 静态方法（类方法）和实例方法**

- **静态方法**用 `static` 修饰，属于类本身，而不属于某个对象实例。可以通过 `类名.静态方法()` 的方式调用，也可以通过对象实例调用（虽然不推荐）。
- **实例方法**属于类的实例，需要通过对象实例调用。

**3. 静态方法的限制**

- 静态方法不能直接访问实例属性或实例方法，因为它们属于类本身，与具体的对象实例无关。
- 静态方法可以访问**静态属性**和调用**其他静态方法**。

**4. 实例方法的权限**

- 实例方法可以访问类的**实例属性**，也可以调用**实例方法**。
- 实例方法还可以访问**静态属性**和调用**静态方法**，因为静态内容是属于整个类的，可以通过类名直接访问。

**5. 静态属性和实例属性**

- **静态属性**用 `static` 修饰，属于类本身，所有对象共享同一个静态属性。可以通过类名或对象实例来访问，但推荐通过类名访问。
- **实例属性**属于每个对象实例，具有独立的值。

**6. 类方法和实例方法的相互调用**

- 在类方法（静态方法）中，不能直接调用实例方法或访问实例属性，因为类方法不属于某个具体的对象实例。
- 在**实例方法**中，可以通过使用 `类名.静态方法()` 的方式调用类方法，也可以通过 `this` 引用静态属性或静态方法。
...
1. **运算符重载的目的：**
   ...
   - 运算符重载用于让自定义类型（如类或结构体）像内置类型一样使用运算符。
...
3. **运算符重载的限制：**
   ...
   - 操作数个数不可改变：
     ...
     - 二元运算符（如 `+`、`-`）必须有两个操作数。
   - 优先级不可改变：
     - 例如，`*` 的优先级高于 `+`，这一规则不会因重载而改变。
   - 结合性不可改变：
     - 例如，`=` 是右结合运算符，`+` 是左结合运算符，这些规则不可改变。
...
5. **不能重载的运算符：**
   ...
   - `::`（域运算符）、`.*`（成员指针访问运算符）、`sizeof`（大小运算符）、`?:`（条件运算符）、`#`（预处理符号）。

6. **运算符重载注意事项：**

   - 不要改变运算符的常规用途或含义，例如不要让 `+` 做减法。
   - 避免重载过于复杂的运算符，保持代码可读性。
...
str本身是一个指针，函数参数也是一个指针，所以这是按值传递，如果我们想按址传递，那么我们应该定义一个指向str的指针即指针的指针     

### **知识点总结**

**1. 指针参数传递的特点**

- C/C++ 中，函数参数是按值传递的，指针作为参数时，传递的是指针的值（地址）。
- 如果需要在函数中修改指针本身（如指向新分配的内存），必须使用指针的指针。

**2. `malloc` 动态内存分配**

- `malloc` 返回的是一块未初始化的内存区域，其地址需要赋值给指针变量。
- 使用 `malloc` 分配的内存，需配合 `free` 释放。

**3. `strcpy` 的使用**

- `strcpy` 函数用于字符串复制，目标地址必须是有效的、可写的内存。
- 如果目标指针是 `NULL` 或未分配内存，则调用会导致段错误。

**4. 动态内存管理的最佳实践**

- 确保动态分配的内存被正确释放，避免内存泄漏。
- 在释放指针后，将指针置为 `NULL`，防止悬空指针问题。
...
1. 运算符重载必须作为类的成员函数或友元函数。
2. 一些运算符（如 `=`、`[]`、`()` 和 `->`）必须作为成员函数进行重载。
3. 运算符的优先级和结合性不能通过重载改变。
4. 逻辑短路（`&&` 和 `||` 的短路行为）在重载中不会保留。
...
B选项说法错误，基类中说明了虚函数后，派生类中对应的函数也默认是虚函数，可以不用加 virtual 进行说明，为增强可读性，通常在派生类中也加入virtual关键字。
C选项说法正确，友元函数不能是虚函数，因为友元函数不可以继承，对于没有继承特性的函数，因而是没有虚函数。但是虚函数可以是另一个类的友元函数。
D选项说法正确，如果一个类有纯虚函数，则该类称为抽象类，是不能实例化的。所以派生类要实例化，必须对纯虚函数进行实现。

### **知识点总结**

**虚函数的基本概念**

- 虚函数用于支持**运行时多态**，通过基类的指针或引用调用派生类的重写函数。
- 使用 `virtual` 修饰成员函数可以将其声明为虚函数。

**虚函数的特性**

1. 动态绑定：
   - 调用虚函数时，会根据实际对象类型（运行时类型）来决定调用哪一个函数。
2. 继承性：
   - 派生类中继承基类虚函数时，无需再次声明 `virtual`，它会自动成为虚函数。
3. 不能是静态成员函数：
   - 因为静态函数与对象无关，而虚函数需要通过对象实现动态绑定。
4. 可以是友元函数：
   - 虚函数可以被声明为另一个类的友元函数。
...
- 特点：
  ...
  - 基类中有纯虚函数时，该类为**抽象类**，无法实例化。
  - 如果派生类需要实例化，必须实现所有纯虚函数。
  - 若派生类未实现所有纯虚函数，则它也是抽象类，不能实例化。
...
C++语言中，若只需要通过一个成员函数读取数据成员的值，而不需要修改它，则应在函数头的后面加上static关键字。（   ）
...
static表示静态意思 const表示不可更改 

### 知识点总结：

1. **`static` 关键字的用途**：
   - 用于定义类的静态成员变量或静态成员函数。
   - 静态成员函数不依赖于类的对象，可以直接通过类名访问，且只能访问类的静态成员变量，不能访问非静态成员变量。
2. **正确关键字：`const`**：
   - 如果成员函数只需要读取数据成员的值，而不需要修改它，应在成员函数的声明和定义中添加 **`const`** 关键字。
   - `const` 成员函数的作用是保证该函数不能修改类的非静态数据成员，增强代码的安全性。

## 例题67

下面有关析构函数和虚函数的说法，错误的是？

A.析构函数可以在对象生命期结束时释放对象动态分配的内存资源

B.析构函数是特殊的类成员函数，它的名字和类名相同，前面加了~符号，没有返回值，没有参数，不能随意调用，但是可以重载
...
- 析构函数
  ...
  - **没有返回值**，也**不能有参数**。
  - **不能被重载**，每个类只能有一个析构函数。
  - 在对象生命周期结束时自动调用，通常用于释放动态分配的资源。
  - 用户不能直接调用析构函数（除了特定场景下通过显式调用手动销毁对象，例如使用 `delete` 操作符）。

2. **析构函数与虚函数**：

- 析构函数可以是虚函数。如果一个类可能作为基类使用，推荐将其析构函数声明为虚函数，以确保当通过基类指针删除对象时，调用的是正确的子类析构函数，避免资源泄露。

3. **虚函数的动态绑定**：

- 虚函数支持 **动态绑定**，即在运行时确定调用哪个函数。
- 当基类定义了虚函数，且子类重写该虚函数时，通过基类指针或引用调用函数时会执行子类的实现。
...
- 在现代计算机中，整数通常用 **补码（Two's Complement）** 表示。
- 补码特性：
  - 正数的补码与其原码相同。
  - 负数的补码是其绝对值对应正数的二进制取反后加1。
  - 补码范围：
    - 对于 32 位整数：范围是 `[-2^31, 2^31-1]`（即 `[-2147483648, 2147483647]`）。
    - 超过范围时会发生溢出，但补码运算具有循环性（模运算的效果）。
...
- 加法和减法溢出：
  - 在补码规则下，即使发生溢出，算术运算仍然遵循规则。
  - 例如，在 4 位整数范围（[-8, 7]）中：
    - `-8 + (-5) = -13`，结果实际为 `-13 + 16 = 3`。
    - 继续计算：`3 - (-8) = 11`，结果实际为 `11 - 16 = -5`。
...
a[4]表示 4 个对象，而*p[5]表示五个指针，并没有分配内存空间，因此不会调用构造函数和析构函数。如果需要生成，需要分配内存空间。

### **知识点总结**

1. **数组对象的构造函数调用**

   - 当数组声明时，**数组中每个对象都会调用构造函数**。
   - 如果数组长度为 `n`，构造函数会被调用 `n` 次。

2. **指针数组的构造函数调用**

   - 声明指针数组时，数组中的元素只是指针，而不是实际的类对象。
   - 仅分配了指针的内存空间，不会调用构造函数。

3. **动态内存分配和构造函数**

   - 如果指针指向动态分配的对象，则在动态分配时会调用构造函数。

   - 示例：

     ```cpp
     TestClass* p = new TestClass[5]; // 动态分配数组，调用构造函数 5 次
     ```
...
- **Traits** 是一种基于模板的设计模式，常用于泛型编程。
- 用于在编译期根据模板参数类型，选择对应的特性或功能实现。

**Traits 的组成**

1. **`enum` 或常量**：定义某些静态属性（如布尔值）。
2. **`typedef` 或 `using`**：定义类型别名，与模板参数相关。
3. **模板偏特化**：为特定类型提供特殊实现。
...
1. 类型属性检测：
   - 如检测某类型是否为整型、浮点型等。
   - 例如：`std::is_integral`、`std::is_floating_point`。
2. 选择算法或逻辑：
   - 根据类型特性选择最优实现。
3. 类型重定义：
   - 为不同类型提供统一的接口。
...
template <typename T>
struct TypeTraits {
    static const bool is_integral = false; // 默认类型非整型
};
...
// 测试
int main() {
    std::cout << TypeTraits<double>::is_integral << "\n"; // 输出: 0 (false)
    std::cout << TypeTraits<int>::is_integral << "\n";    // 输出: 1 (true)
    return 0;
}
```
...
1. **数组地址和指针的区别：**
   - `&a` 的类型是 `int (*)[5]`，表示一个指向数组的指针。
   - `a` 的类型是 `int *`，表示指向数组第一个元素的指针。
   - `&a + 1` 表示跳过整个数组的大小，而 `a + 1` 表示跳过一个元素的大小。
2. **指针偏移的单位：**
   - 指针的偏移量是基于指针类型的大小。例如：
     - 对 `int *` 指针，每次偏移移动 `sizeof(int)` 字节。
     - 对 `int (*)[5]` 指针，每次偏移移动 `5 * sizeof(int)` 字节。
3. **解引用操作：**
   - 指针的解引用会访问指针指向地址处的值。必须保证访问的地址有效，否则可能引发未定义行为。
4. **指针越界：**
...
**短路运算**指的是在逻辑运算中，如果左边的操作数已经能决定最终结果，右边的操作数就不再被计算。C++ 中有两种短路运算符：
...
- 如果 `A` 为 `false`（即 `0`），则结果无论 `B` 是什么都会是 `false`（因为 `false && anything` 都是 `false`）。
- 如果 `A` 为 `true`，则继续计算 `B` 的值，结果依赖于 `B` 的值。

因此，在 `A && B` 中，如果 `A` 为 `false`，右边的 `B` 不会被计算（称为短路）。

**1.2 逻辑或运算符 (`||`)**

对于表达式 `A || B`：

- 如果 `A` 为 `true`，则结果无论 `B` 是什么都会是 `true`（因为 `true || anything` 都是 `true`）。
- 如果 `A` 为 `false`，则继续计算 `B` 的值，结果依赖于 `B` 的值。

因此，在 `A || B` 中，如果 `A` 为 `true`，右边的 `B` 不会被计算（同样是短路）。

2. **赋值运算符 (`=`) 的行为**

在 C++ 中，赋值运算符 `=` 是一个 **右值运算符**，其计算结果是赋值后的值。换句话说，`a = b` 的结果是 `b` 的值，并且赋值会在表达式中作为一个值参与运算。

例如：

```
c++复制代码int x = 3;
int y = (x = 5);  // x 变为 5，y 变为 5
```

在这个例子中，`x = 5` 会先被计算，结果是 `5`，然后这个 `5` 被赋给 `y`。

3. **逻辑表达式中的赋值**

在 C++ 中，可以将赋值表达式用于逻辑表达式中，且赋值表达式会有一个值。例如：

```
c++复制代码int x = 0;
if (x = 5) {
    // 因为 x = 5 的结果是 5，且 5 被视为 true，因此这里的代码会执行
}
```

此时，`x = 5` 会将 `x` 的值改为 `5`，并且 `x = 5` 的结果为 `5`，由于 `5` 被视为 `true`，`if` 语句会执行。

**需要注意：** 赋值表达式常常是逻辑运算中的潜在错误源，尤其在条件语句中，不小心使用赋值符号 `=` 而非比较符号 `==` 时，会导致程序行为与预期不符。

4. **运算符优先级和结合性**

C++ 中有一套运算符的优先级规则，决定了在表达式中各个运算符的执行顺序。需要特别注意的是：

- **赋值运算符**（`=`）的优先级较低，通常在表达式中会在其他运算之后执行。
- **逻辑与运算符（`&&`）** 和 **逻辑或运算符（`||`）** 的优先级高于赋值运算符，但是低于算术运算符。
...
指针数组是一个数组，数组中的每个元素都是一个指针。在本例中，`const char *c[]` 是一个指向 `const char` 的指针数组，其中每个元素指向一个字符串常量。

```c++
const char *c[] = {"HELLO", "NEW", "WORLD", "SAYHI"};
```

`c` 是一个指针数组，`c[0]` 指向 `"HELLO"`，`c[1]` 指向 `"NEW"`，依此类推。

2. **指向指针数组的指针（指针到指针）**

`cp` 是一个指向指针数组的指针数组，即 `cp` 本身是一个指向 `const char**` 类型的指针数组。

```cpp
const char **cp[] = {c + 3, c + 2, c + 1, c};
```

`cp[0]` 指向 `c + 3`（即 `c[3]`），`cp[1]` 指向 `c + 2`（即 `c[2]`），依此类推。

3. **多级指针的解引用**

在多级指针中，我们需要逐级解引用指针。以下是几种常见的解引用方式：

- `**ptr`：解引用两级指针。
- `*ptr`：解引用一级指针。

例如，`cpp` 是 `const char***` 类型，它指向 `cp` 数组的元素，进一步解引用可以访问 `c` 数组中的字符串。

4. **`++` 和 `--` 运算符在指针上的应用**

- **`++cpp`**：先将 `cpp` 指针增加，指向下一个元素。
- **`\*++cpp`**：先增加 `cpp`，然后解引用它。
- **`--\*cpp`**：先解引用 `cpp`，然后对解引用的结果执行减法操作。

5. **指针数组的访问和偏移**

- **`cpp[-1]`**：指针 `cpp` 向后偏移 1 个元素，相当于 `cp[1]`。
- **`cpp[-2]`**：指针 `cpp` 向后偏移 2 个元素，相当于 `cp[0]`。

在这个例子中，通过 `cpp[-1][-1]` 访问 `cp[1][1]`，也就是 `c + 2` 中的第二个元素，结果是 `"WORLD"`。

6. **指针加偏移**

- `*cpp[-2] + 3` 表示 `c[3]`（即 `"SAYHI"`）加上偏移 3，最终得到 `"HI"`。

7. **输出字符串偏移**

- 对于字符串常量，通过 `+ N` 可以实现指向字符串的偏移。例如，`"WORLD" + 1` 表示从 `"WORLD"` 字符串的第二个字符 `'O'` 开始。
